define(function (require) {
	...
	function replaceSP2013GlobalNav() {
	    if (!commonUI.isSP2013()) { return; }

	    datacontext.getNavigation().then(function (data) {
	        var rootNode = data[0];

	        if (rootNode.children) {
	            var leftNavBar = $('#DeltaPlaceHolderLeftNavBar'),
	                $menuDiv = $('<div id="exerciseSiteMenu"></div>'),
	                $menuUL = $('<ul id="menuList"></ul>');

	            _.each(rootNode.children, function (topLevelNode) {
	                createMenuItem(topLevelNode, $menuUL);
	            });

	            //DOM is built-out so pass it to SP2010 for rendering
	            //SP.UI.AspMenu still exists in SP2013
	            $menuDiv.append($menuUL);
	            leftNavBar.prepend($menuDiv);
	            fixForIE();

                //empty quick launch contents
	            $(".ms-quicklaunchouter").empty();
	            leftNavBar.show();
	        }

	    });

	    function createMenuItem(node, parentUL) {
	        var shouldDisplay = (node.li_attr.visibility === "Anonymous" ||
                    (node.li_attr.visibility === "Authenticated" && isCurrentUserAuthenticated) ||
                    !node.li_attr.visibility);
	        ...
	        var $listItem = $('<li>'),
	        	nodeHasChildren = (node.children && node.children.length > 0),
	        	$anchorTag = buildAnchorTag(node, nodeHasChildren);

	        $listItem.append($anchorTag);
	        parentUL.append($listItem);

	        if (!nodeHasChildren) {
	            //base case, but after listItem and anchor
	            return;
	        }
	        else {
	            var $childUL = $("<ul></ul>");
	            ...
	            _.each(node.children, function (childNode) {
	                createMenuItem(childNode, $childUL);
	            });
	        }
	    }

	    function buildAnchorTag(node, hasChildren) {
	        var $anchorTag = $('<a></a>'), // $elm will either a <span> or <a> tag
                href = node.li_attr && node.li_attr.url;
	        ...
	        $anchorTag.text(node.text);

	        if (hasChildren) {
	            $anchorTag.attr('name', 'submenu');
	            $anchorTag.attr('class', 'submenu');
	        }
	...
	return {
	    ...
	    replaceSP2013GlobalNav: replaceSP2013GlobalNav
	}    
});
